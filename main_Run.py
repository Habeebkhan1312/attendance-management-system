    ...
    def fill_attendance():
        ...
        if subb == '':
            ...
            def create_csv():
                ...
                csv_name = 'C:/Users/Patan Davood Khan/Downloads/attendance management/Attendance/Manually Attendance'+DB_table_name+'.csv'
            ...
            def attf():
                import subprocess
                subprocess.Popen(
                    r'explorer /select,"C:\Users\Patan Davood Khan\Downloads\attendance management\Attendance\Manually Attendance\-------Check atttendance-------"')
    ...
    if l1 == '':
        ...
        try:
            ...
            while True:
                ret, img = cam.read()
                if not ret:
                    Notification.configure(text="Failed to capture image from webcam", bg="red", fg="white")
                    break
                ...
                # Stop after capturing one image to simplify capture process
                if sampleNum >= 1:
                    break
            ...
            Notification.configure(text=res, bg="SpringGreen3", fg="white", font=('times', 18, 'bold'))

        except Exception as e:
            Notification.configure(text=f"Error: {str(e)}", bg="red", fg="white", font=('times', 18, 'bold'))
...
def subjectchoose():
    def Fillattendances():
        ...
        try:
            ...
            Notifica.configure(text="Model not found. Please train the model.", bg="red", fg="black", font=('times', 15, 'bold'))
        ...
        try:
            while True:
                ret, im = cam.read()
                if not ret:
                    Notifica.configure(text="Failed to access webcam", bg="red", fg="black", font=('times', 15, 'bold'))
                    break
                ...
                for (x, y, w, h) in faces:
                    ...
                    if conf < 70:
                        ...
                        new_row = pd.DataFrame({"Enrollment": [Id], "Name": [name], "Date": [date], "Time": [timeStamp]})
                        attendance = pd.concat([attendance, new_row], ignore_index=True)
                        
                        # Draw rectangle and log detection
                        cv2.rectangle(im, (x, y), (x + w, y + h), (0, 255, 0), 2)
                        print(f"Detected: {name} with ID: {Id}")
                    else:
                        print("Unknown face detected.")

                # Save the frame for debugging or visualization purposes
                if not os.path.exists("CapturedFrames"):
                    os.makedirs("CapturedFrames")
                cv2.imwrite("CapturedFrames/frame.jpg", im)

                # Break condition (adjust as needed)
                if len(attendance) >= 5:  # Stop after capturing 5 valid attendances
                    break
        except Exception as e:
        ...
        filename = f"Attendance/Attendance_{sub}_{timestamp}.csv"
        attendance.to_csv(filename, index=False)
        print(f"Attendance saved to {filename}")
        Notifica.configure(text="Attendance captured successfully!", bg="green", fg="white", font=('times', 15, 'bold'))

    # Frame for subject chooser
    windo = tk.Tk()
    ...
    def log_in():
        ...
        if username == 'habeeb':
            if password == 'habeeb123':
                ...
                cs = 'C:Users/Patan Davood Khan/Downloads/attendance management/StudentDetails/StudentDetails.csv'
